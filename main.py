...
class WordWizard:
    ...
    def count_words(self) -> int:
        words = re.findall(r'\b\p{L}+\b', self.text, re.UNICODE)  # znajdujemy wszystkie słowa w tekście
        return len(words)  # zwracamy liczbę słów

    def count_numbers(self) -> int:
        numbers = re.findall(r'\b\p{N}+(?:\.\p{N}+)?(?:[eE][+-]?\p{N}+)?\b', self.text, re.UNICODE)  # znajdujemy liczby w różnych formatach
        return len(numbers)  # zwracamy liczbę liczb

    def words_starting_with_capital(self) -> list[str]:
        return re.findall(r'\b\p{Lu}\p{L}*\b', self.text, re.UNICODE)  # znajdujemy słowa rozpoczynające się wielką literą
    ...
    def words_with_sequences(self, sequences: list[list[str]]) -> dict[str, list[str]]:
        ...
        for sequence in sequences:
            ...
            pattern = r'\b\p{L}*?' + r'.*?'.join(map(re.escape, sequence)) + r'\p{L}*?\b'
            results["".join(sequence)] = re.findall(pattern, self.text, re.UNICODE | re.IGNORECASE)  # wyszukujemy pasujące słowa
        return results

    def unique_sorted_words(self) -> list[str]:
        words = re.findall(r'\b\p{L}+\b', self.text, re.UNICODE)  # znajdujemy wszystkie słowa
    ...
    def word_frequency(self) -> dict[str, int]:
        ...
        pattern = re.findall(r'\b\p{L}+\b|\b\p{N}+(?:\.\p{N}+)?(?:[eE][+-]?\p{N}+)?\b|[^\w\s]', self.text, re.UNICODE)
